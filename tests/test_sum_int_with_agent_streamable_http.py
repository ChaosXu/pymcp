#!/usr/bin/env python3
"""
通过agent使用MCP的测试脚本 (Streamable HTTP方式)
"""
    ...
    while time.time() - start_time < timeout:
        try:
            async with httpx.AsyncClient() as client:
                ...
                # Streamable HTTP服务器会返回405 (Method Not Allowed)而不是200
                # 因为它只接受POST请求，所以检查状态码是否不是502
...
if LANGCHAIN_AVAILABLE:
    async def test_agent_with_mcp_streamable_http():
        """测试通过agent使用MCP (Streamable HTTP方式)"""
        # 从环境变量获取配置
        ...
        # 启动MCP服务器 (Streamable HTTP方式)
        print("Starting MCP server with Streamable HTTP transport...")
        server_process = subprocess.Popen([
            sys.executable, "src/mcp_server/sum_int.py", "streamable-http"
    ...
    def main():
        ...
        print("=== Agent with MCP Integration Test (Streamable HTTP) ===")
        base_url = os.environ.get("LLM_BASE_URL")
        model = os.environ.get("LLM_MODEL")
        ...
        try:
            asyncio.run(test_agent_with_mcp_streamable_http())
        except Exception as e:
